...
export function AnimatedNumber({
	...
	useEffect(() => {
		...
		const padPrev = prevChars.length < maxLen
			? Array(maxLen - prevChars.length).fill("").concat(prevChars)
			: prevChars
		const padNext = nextChars.length < maxLen
...
function AnimatedChar({ char, from }: { char: string; from: string }) {
	...
	return (
		<span className="relative inline-block h-[1lh] overflow-hidden">
			...
			{/* New digit slides in */}
			<span
				className="absolute left-0 top-0 inline-block animate-[slideIn_200ms_ease-out_forwards]"
				style={{
					// @ts-expect-error -- CSS custom property
					"--dir": goingUp ? "100%" : "-100%",
				}}
